...
def calculate_sequence(sequence: List[int]) -> List[int]:
    ...
    all_sequences[-1].append(0)
    for curr_rank in range(len(all_sequences) - 2, -1, -1):
        to_append = all_sequences[curr_rank + 1][-1] + all_sequences[curr_rank][-1]
        all_sequences[curr_rank].append(to_append)
...
def solution():
    ...
    for sequence in sequences:
        result += calculate_sequence(sequence)[-1]
    return result
